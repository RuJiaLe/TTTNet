...
from .transformer import OverlapPatchEmbed, Block
...
class De_Block(nn.Module):
    def __init__(self, in_channels, out_channels, img_size, num_heads, mlp_ratio, sr_ratio):
        super(De_Block, self).__init__()

        self.patch_enbed = OverlapPatchEmbed(img_size=img_size, patch_size=3, stride=1, in_chans=in_channels, embed_dim=out_channels)
        
        self.block = Block(dim=out_channels, num_heads=num_heads, mlp_ratio=mlp_ratio, sr_ratio=sr_ratio)
    ...
    def forward(self, in_x):
        B = in_x.shape[0]
        x, H, W = self.patch_enbed(in_x)
        x = self.block(x, H, W)
        x = self.norm(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        out = x + self.conv1x1(in_x)
...
class refine_module(nn.Module):
    def __init__(self):
        ...
        self.ass_t = shunted_t(pretrained=True)
        ...
        self.de_block4 = De_Block(512, 256, img_size=8, num_heads=16, mlp_ratio=4, sr_ratio=1)
        self.de_block3 = De_Block(256, 128, img_size=16, num_heads=8, mlp_ratio=4, sr_ratio=2)
        self.de_block2 = De_Block(128, 64, img_size=32, num_heads=4, mlp_ratio=8, sr_ratio=8)
        self.de_block1 = De_Block(64, 32, img_size=64, num_heads=2, mlp_ratio=8, sr_ratio=8)
        self.de_block0 = De_Block(32, 32, img_size=128, num_heads=2, mlp_ratio=8, sr_ratio=8)
    ...
    def forward(self, x, in_blocks):
        ...
        decoder2 = self.de_block2(self.CBR2(decoder3 + out_block[-3] + in_blocks[1]))
        decoder1 = self.de_block1(self.CBR1(decoder2 + out_block[-4] + in_blocks[0]))
