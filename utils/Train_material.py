    ...
    for i, packs in enumerate(zip(train_data, cycle(val_data))):
        ...
        if i % 100 == 0 or i == total_step:
            logging.info('#Train# {}, Epoch: [{}/{}], Done: {:0.2f}%, Step: [{}/{}], Loss: {:0.4f}'.
                         format(datetime.now().strftime('%m/%d %H:%M'),
                                Epoch, args.total_epoch, (i / total_step) * 100, i,
                                total_step, all_losses / (i * args.clip_len)))
...
def start_train(train_dataloader, val_dataloader, model):
    # tensorboard
    writer = tensorboard.SummaryWriter('./Log_tensorboard/train')

    # log
    logging.basicConfig(filename=args.log_dir + '/train_log.log', format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]',
